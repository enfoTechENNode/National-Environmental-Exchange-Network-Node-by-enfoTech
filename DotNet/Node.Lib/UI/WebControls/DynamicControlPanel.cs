...
namespace Node.Lib.UI.WebControls
{
   ...
   ToolboxData("<{0}:DynamicControlsPanel runat=server></{0}:DynamicControlsPanel>")]
    public class DynamicControlPanel : PlaceHolder
    {
        ...
        /// <summary>
        /// Occurs when the DynamicControlPanel is about to restore the child controls from ViewState
        /// </summary>
        public event EventHandler PreRestore;
        /// <summary>
        /// Occurs after the DynamicControlPanel has restored the child controls from ViewState
        /// </summary>
        ...
        private Pair PersistChildStructure(Control control, string controlCollectionName)
        {
            ...
            if (control.ID == null)
            {
                if (ControlsWithoutIDs == HandleDynamicControls.ThrowException)
                    throw new NotSupportedException("DynamicControlPanel does not support child controls whose ID is not set, as this may have unintended side effects: " + control.GetType().ToString());
        ...
        protected override void Render(System.Web.UI.HtmlTextWriter writer)
        {
            if (HttpContext.Current == null)
            {
                string designTimeHTML = "<p style=\"font-family:Arial; font-size:7pt; color:#666666;\">" +
                    DateTime.Now.ToString("d") + "</p>";
