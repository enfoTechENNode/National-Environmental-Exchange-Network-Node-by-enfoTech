...
<html>
	<head>
		...
		<title>Exchange Network Node</title>
      	...
      	<body >
          <form action="/Node.Client/Page/WebMethods/Download.do?version=VER_20" method="POST" name="form1" enctype="multipart/form-data">
                            ...
                            <td>
                              <table width="100%">
                                <tr>
                                  ...
                                  <td>
                                    <select name="nodeAddress3">
                                      <%
                                      	String[] clientURLs = DownloadBean.getClientURLs_V2();
                                      	boolean	isSelected = false;
                                       if (clientURLs != null) {
                                         for (int i = 0; i < clientURLs.length; i++) {
                                           if (clientURLs[i].equals(DownloadBean.getNodeAddress3()))
                                             isSelected = true;
                                         }
                                       }
                                       if (isSelected) {
                                ...
                                <tr>
                                  ...
                                  <td><eftwc:HighlightTextBox id="nodeAddress4" size="80" text="<%=DownloadBean.getNodeAddress4()%>" /></td>
                                </tr>
                              </table>
                            </td>
                          </tr>
                            ...
                            <td>
                              <table width="100%">
                                      ...
                                      <tr>
                                        ...
                                        <td><eftwc:HighlightTextBox id="file1" size="40" text="<%=DownloadBean.getFile1()%>" />
                                          <span class="formFldLabel">File Type:                                          
			                                    <select name="fileType1">
													<%
														String[] fileType = {"XML", "Flat", "Bin","ZIP","ODF","OTHER"};
														boolean isSelectedFileType1 = false;
														for (int i = 0; i < fileType.length; i++) {
															if (fileType[i].equals(DownloadBean.getFileType1()))
																isSelectedFileType1 = true;
														}
														for (int i = 0; i < fileType.length; i++) {
															if (fileType[i].equals(DownloadBean.getFileType1())) {
															%>
															<option selected="selected"><%=fileType[i]%></option>
															<%
																} else {
															%>
                                      ...
                                      <tr>
                                        <td class="formFldLabel" nowrap="nowrap">File Name:</td>
                                        <td><eftwc:HighlightTextBox id="file2" size="40" text="<%=DownloadBean.getFile2()%>" />
                                          <span class="formFldLabel">File Type:                                          
			                                    <select name="fileType2">
													<%
														boolean isSelectedFileType2 = false;
														for (int i = 0; i < fileType.length; i++) {
															if (fileType[i].equals(DownloadBean.getFileType2()))
																isSelectedFileType2 = true;
														}
														for (int i = 0; i < fileType.length; i++) {
															if (fileType[i].equals(DownloadBean.getFileType2())) {
															%>
                                      ...
                                      <tr>
                                        <td class="formFldLabel" nowrap="nowrap">File Name:</td>
                                        <td><eftwc:HighlightTextBox id="file3" size="40" text="<%=DownloadBean.getFile3()%>" />
                                          <span class="formFldLabel">File Type:                                          
			                                    <select name="fileType3">
													<%
														boolean isSelectedFileType3 = false;
														for (int i = 0; i < fileType.length; i++) {
															if (fileType[i].equals(DownloadBean.getFileType3()))
																isSelectedFileType3 = true;
														}
														for (int i = 0; i < fileType.length; i++) {
															if (fileType[i].equals(DownloadBean.getFileType3())) {
															%>
