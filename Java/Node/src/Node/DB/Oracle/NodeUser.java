...
*/
  ...
  public int verifyEmail (String email)
  {
    ...
    ResultSet rs = null;
    try {
      String sql = "select b.* from " + this.SysEmailTableName + " a, "+ this.SysUserEmailTableName + " b where a.EMAIL_ID = b.EMAIL_ID and "+this.SysEmail+" = '"+email.trim()+"'";
      db = this.GetNodeDB();
      rs = db.GetResultSet(sql);
      if (rs != null && rs.last() && rs.getRow() == 1) {
    	userId = rs.getInt(this.SysUserInfoUserID);
      }
    } catch (Exception e) {
      this.LogException("Could Not verify Email: " + e.toString());
    } finally {
      try {
        if (rs != null)
          rs.close();
